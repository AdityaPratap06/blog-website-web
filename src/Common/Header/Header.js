import { Box, Flex, Grid, GridItem, IconButton, Input, InputGroup, InputLeftAddon, InputRightAddon, Menu, MenuButton, MenuItem, MenuList, Portal, Text } from "@chakra-ui/react";
                ...
                <Box w="100%">
                    <Flex w="100%" py={5} flexDir={{ base: "column", md: "row" }} justify={{ base: "center", md: "space-between" }} align={"center"}>
                        ...
                        <Flex w={{ base: "100%", md: "50%" }} mt={{ base: 5, md: 0 }} justify={"center"}>
                            <IconButton display={{ base: "initial", md: "none" }} icon={<HamburgerIcon />} mr={10} />
                    ...
                    <Grid templateColumns={`repeat(${ITEMS?.length}, 1fr)`} gap={0} color="white" fontWeight={"semibold"}>
                        {map(ITEMS, item => (
