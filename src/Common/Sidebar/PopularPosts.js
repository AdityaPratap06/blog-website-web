            ...
            {popularPosts && popularPosts.length ?
                <Box p={5}>
                    {map(popularPosts, (post, index) => (
                        index === 0 ?
                            <Flex flexDir={"column"} h={{ base: 170, md: 230, lg: 200 }} w="100%" p={5} align={"start"} justify={"end"} color="white" bgImage={`url(${post.intro.url})`} backgroundSize={"cover"} bgRepeat={"no-repeat"}>
                            ...
                            <Flex w={"100%"} h={"70px"} mt={5}>
                                <Image h="100%" w={{ base: "40%", md: "20%", lg: "40%" }} aspectRatio={16 / 9} src={post.intro.url} alt="" />
